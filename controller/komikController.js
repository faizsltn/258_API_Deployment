...
async function create(req, res) {
    try {
        ...
        // Masukkan relasi genre secara langsung ke tabel pivot KomikGenre
        if (genre_ids && genre_ids.length > 0) {
            const komikGenres = genre_ids.map(genre_id => ({
                komik_id: komik.id,
                genre_id: genre_id
            }));

            // Pakai bulkCreate ke tabel pivot
            if (db.sequelize.models.KomikGenre) {
                await db.sequelize.models.KomikGenre.bulkCreate(komikGenres);
            } else {
                // Opsi cadangan pakai method setGenre bawaan Sequelize
...
async function update(req, res) {
    try {
        ...
        if (genre_ids) {
            if (db.sequelize.models.KomikGenre) {
                // Hapus relasi lama
                await db.sequelize.models.KomikGenre.destroy({
                    where: { komik_id: id }
                });
                // Tambah relasi baru
                const komikGenres = genre_ids.map(genre_id => ({
...
async function remove(req, res) {
    try {
        ...
        // Hapus juga data relasi di tabel pivot jika ada
        if (db.sequelize.models.KomikGenre) {
            await db.sequelize.models.KomikGenre.destroy({
                where: { komik_id: id }
            });
